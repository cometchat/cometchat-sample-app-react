...
const reducers = (state = intialState, action) => {
  ...
  switch (action.type) {
    ...
    case "updateMessageDelivered":
      ...
      console.log(
        "kshitiz",
        "single message received index : " + action.deliveredAt
      );
      if (msgDindex != -1) {
        //uid is present
        ...
        if (messageIndex != -1) {
          console.log(
            "kshitiz",
            "single message received index : " + messageIndex
          );
          console.log("kshitiz");

          const newMessageState = {
            ...copyPSMessageState.message[messageIndex],
          ...
          const PSMessageState = update(state, {
            messages: { msgDindex: { $set: [copyPSMessageState] } }
          });
          console.log(
            "message delivered single : ",
            JSON.stringify(PSMessageState)
          );
    ...
    case "updateMessageRead":
      ...
      console.log(
        "kshitiz",
        "single message received index : " + action.readAt
      );

      if (msgReadIndex != -1) {
        //uid is present
        var copyPSMessageState = Object.assign(
          {},
          state.messages[msgReadIndex]
        );
    ...
    case "updateMessage":
      ...
      let index = newState.messages.findIndex(
        userMessage => userMessage.muid === action.uid
      );
      ...
      if (index != -1) {
        ...
        const PSMessageState = update(state, {
          messages: { index: { $set: [copyPSMessageState] } }
        });
    ...
    case "updateMessageList":
      ...
      if (index1 != -1) {
        ...
        const NPLMessageState = update(state, {
          messages: { index: { $set: [copyMessageState] } }
        });
        ...
        const NPMessageState = update(state, {
