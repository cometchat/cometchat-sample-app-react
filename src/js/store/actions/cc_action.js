...
export const handleMessage = (msg, dispatch) => {
  if (msg.receiverType == "user") {
    dispatch(updateMessage(msg.sender.uid, msg, "user"));
  } else {
    dispatch(updateMessage(msg.receiver, msg, "group"));
  }
};
...
export const handleMessageDelivered = (message, dispatch) => {
  ...
  if (message.receiverType == "user") {
    dispatch(
      updateMessageDelivered(message.sender.uid,message.messageId,message.timestamp," message ")
    );
  } else {
    dispatch(
      updateMessageDelivered(message.receiver,message.messageId,message.timestamp," message ")
    );
  }
};
...
export const handleMessageRead = (message, dispatch) => {
  if (message.receiverType == "user") {
    dispatch(updateMessageRead(message.sender.uid,message.messageId,message.timestamp,"message","user"));
  } else {
    dispatch(updateMessageRead(message.receiver,message.messageId,message.timestamp,"message","group"));
  }
};

export const updateMessageRead = (user, msgid, timestamp, tag, userType) => {
  return {
...
export const updateMessageList = (message, val, tag, loggedInUser) => {
  return {
    type: "updateMessageList",
    messages: message,
    uid: val,
    tags: tag,
    logggedInUid: loggedInUser
  };
};

//getUserMessageHistory
export const getUserMessageHistory = (utype, uid, limit = 50, loggedInUser) => {
  ...
  return dispatch => {
    messageRequest.fetchPrevious().then(messages => {
      ...
      return dispatch(updateMessageList(messages, uid, utype, loggedInUser));
    });
  };
};
