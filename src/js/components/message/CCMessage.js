...
class CCMessage extends Component {
  ...
  render() {
    ...
    msg.avatar = utils.CheckEmpty(this.props.msgData.sender.avatar)
      ? this.props.msgData.sender.avatar
      : Userthumbnail;

    const imageViewerModal = this.state.showModal ? (
      <ImageViewerModal
        image_src={this.state.imageUrl}
    ...
    switch (this.props.msgData.category) {
      case CometChat.CATEGORY_CALL:
        return (
          ...
          <MessageCall
            msg={msg}
            openImageViewer={image => this.openModalHandler.bind(this, image)}
          />
        );
        break;
          ...
          <div className="incoming_msg" onClick={props.openImageViewer(image)}>
            ...
            <div className="received_msg">
              <div className="received_withd_msg">
                <img
                  class="color-dark-tint-font border-radius-no-bottom-left"
                  src={props.msg.data.url}
                />
                <span className="time_date color-light-tint-font">
                  {util.convertStringToDate(props.msg.sendAt)}
                </span>
              </div>
            </div>
          </div>
        ...
        return (
          <div className="incoming_msg">
            ...
            <div className="received_msg">
              <div className="received_withd_msg">
                <a
                  href={props.msg.data.url}
                  download={props.msg.msgId}
                  target="_blank"
                >
                  <p class="color-light-tint color-dark-tint-font border-radius-no-bottom-left">
                    <div
                      class="file_icon"
                      dangerouslySetInnerHTML={{ __html: icon_msg_file }}
                    />

                    <div class="file_name">{props.msg.data.url}</div>
                  </p>
                </a>
                <span className="time_date color-light-tint-font">
                  {util.convertStringToDate(props.msg.sendAt)}
  ...
  if (props.msgData.receiverType != "group") {
    if (props.msgData.readAt != undefined) {
      messageStatus = (
        <span
          mesasgeStatus="read"
          className="time_date color-light-tint-font"
          dangerouslySetInnerHTML={{ __html: icon_read }}
        />
      );
    } else {
      if (props.msgData.deliveredAt != undefined) {
        messageStatus = (
          <span
            mesasgeStatus="delivered"
            className="time_date color-light-tint-font"
            dangerouslySetInnerHTML={{ __html: icon_msg_delivered }}
          />
        );
      } else {
    ...
    case CometChat.MESSAGE_TYPE.IMAGE:
      {
        return (
          <div
            ...
            onClick={props.openImageViewer(props.msg.data.url)}
          >
