...
class CCMessageHeader extends Component {
  ...
  render() {
    ...
    if (this.props.profile.type == "user") {
      var userdata = this.props.userList.find(
        user => user.uid === this.props.profile.id
      );
      profileData.name = userdata.name;
      profileData.avatar = utils.CheckEmpty(userdata.avatar)
        ? userdata.avatar
        : Userthumbnail;
      ...
      var groupData = this.props.groupList.find(
        group => group.guid === this.props.profile.id
      );
      profileData.name = groupData.name;
      profileData.avatar = utils.CheckEmpty(groupData.icon)
        ? groupData.icon
        : Groupthumbnail;
      profileData.status = groupData.description;

      showProfile = this.state.showModalProfile ? (
        <GroupProfileModal
          groupData={groupData}
    ...
    return (
      <Row className="ccMessageHeader">
        ...
        <Col
          lg={7}
          className="cc-no-padding h-100"
          onClick={this.showModal.bind(this)}
          style={{ cursor: "pointer" }}
        >
          <span className="messageHeaderAvatar">
            <img
              className="userAvatar img-circle"
        ...
        <Col lg={5} className="cc-no-padding h-100">
          <div className="ccMessageHeaderMenu">
            <span
              className="ccmessageHeaderIcon "
              dangerouslySetInnerHTML={{ __html: icon_audio }}
              onClick={this.initiateCall.bind(
                this,
                "audio",
                profileData.name,
                profileData.avatar
            ...
            <span
              className="ccmessageHeaderIcon "
              dangerouslySetInnerHTML={{ __html: icon_video }}
              onClick={this.initiateCall.bind(
                this,
                "video",
                profileData.name,
            ...
            <span
              className="ccmessageHeaderIcon "
              dangerouslySetInnerHTML={{ __html: icon_more }}
            />
          </div>
        </Col>
      </Row>
