...
class CCMessageBox extends Component {
  ...
  componentWillMount() {
    this.fetchfirstTimeMessage(
      this.props.activeUser.type,
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (nextProps.activeUser != this.props.activeUser) {
      this.fetchfirstTimeMessage(
        nextProps.activeUser.type,
        nextProps.activeUser.id
      );
...
const mapDispachToProps = dispatch => {
  return {
    getMessage: (uType, uid, limit) =>
      dispatch(actionCreator.getUserMessageHistory(uType, uid, limit))
  };
};
