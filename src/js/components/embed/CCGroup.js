...
class CCGroups extends Component {
    ...
    switch (group.type) {
      case "public":
        ...
        if (group.hasJoined) {
          ...
          actionCreator.joinPublicGroup(group).then(groupData => {
      ...
      case "private":
        ...
        if (group.hasJoined) {
          ...
          actionCreator.joinPrivateGroup(group).then(groupData => {
            console.log("Joined Group", groupData);
            this.props.updateGroupDetailsJoined(group);
            this.props.showMessageEvent();
          });
  ...
  render() {
    ...
    const passwordModal = this.state.showPasswordModal ? (
      <PasswordRequestModal
        group={this.props.groupData}
        joinGroup={this.joinGroup.bind(this)}
        close={this.hidePasswordModal.bind(this)}
      />
    ) : null;
          ...
          <Row className={classVar.join(" ")}>
            <span className="sidebarUserListItemAvatar">
              <img
                ...
                src={this.props.avt != false ? this.props.avt : Groupthumbnail}
                height="40px"
                width="40px"
              />
            </span>
...
const mapDispachToProps = dispatch => {
  return {
    updateGroupDetailsJoined: group =>
      dispatch(actionCreator.updateGroupJoined(group))
  };
};
