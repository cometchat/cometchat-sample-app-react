...
class CCLeftSidebarHeader extends Component {
  ...
  render() {
    ...
    const createGroup = this.state.showCreateModal ? (
      <GroupCreateModal close={this.hideGroupCreateModal.bind(this)} />
    ) : null;
    return (
      <Row className="sidebarHeader">
        <div>
          <span class="font-title color-font-title size-title">
            {translate[title]}
          </span>
          ...
          <OverlayTrigger
            ...
            rootClose
            placement="bottom"
            overlay={popoverClickRootClose(this.handleLogoutClick)}
          >
            <div
          ...
          <div
            className="header-icon margin-right-10"
            onClick={this.handleCreateNewGroup.bind(this, this.props.activeTab)}
            dangerouslySetInnerHTML={{ __html: iconNewMessage }}
          />
