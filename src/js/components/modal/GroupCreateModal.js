...
class GroupCreateModal extends React.PureComponent {
  ...
  handleChange = event => {
    console.log(
      "handle changes : " + event.target.id + " : " + event.target.value
    );
    this.setState({
  ...
  validateForm() {
    return (
      this.state.guid.length > 0 &&
      this.state.group_name.length > 0 &&
      (this.state.groupType === CCManager.GroupType.PASSWORD
        ? this.state.group_password.length > 0
        : true)
    );
  }
  ...
  render() {
    const passwordView =
      this.state.groupType === CCManager.GroupType.PASSWORD ? (
        <Col lg={12}>
          <FormGroup controlId="group_password">
                ...
                <Col lg={12}>
                  <FormGroup>
                    ...
                    >
                      ...
                      <option value={CCManager.GroupType.PASSWORD}>
                        Password
                      </option>
                      <option value={CCManager.GroupType.PRIVATE}>
                        Private
                      </option>
                    </select>
                  </FormGroup>
              ...
              <Row>
                <Col lg={6}>
                  <Button
                    ...
                    variant="primary"
                    size="lg"
                    className="createGroupButtonCancel"
                  >
                    Cancel
                  </Button>
                </Col>
