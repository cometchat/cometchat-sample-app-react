...
class GroupProfileModal extends React.PureComponent {
  constructor(props) {
    ...
    this.groupMemberRequest = CCManager.getGroupMembersRequestBuilder(
      this.props.groupData.guid,
      100
    );
  }
  ...
  render() {
    console.log(
      "members length group members : " + this.state.groupMembers.length
    );

    let groupLoggedinUser = this.state.groupMembers.find(
      user => user.uid == this.props.loggedInUser.uid
    );

    const deleteGroup =
      groupLoggedinUser == undefined ? null : groupLoggedinUser.scope ==
        "admin" ? (
        <Row className="profileNameRow">
          <Col className="groupProfileLeaveGroupText" lg={10} xs={8}>
            Delete Group{" "}
          </Col>
          <Col
            className="groupProfileLeaveButton"
            lg={1}
            xs={2}
            onClick={this.props.close}
            dangerouslySetInnerHTML={{ __html: icon_delete_group }}
          />
        </Row>
      ) : null;
      ...
      <Modal>
        <div className="groupProfileModal h-100pr">
          <div class="groupProfileModelHeader">
            <Col
              className="groupProfileCloseButton"
              lg={1}
              xs={2}
              onClick={this.props.close}
              dangerouslySetInnerHTML={{ __html: icon_clear }}
            />
            <Col className="groupProfileTitle" lg={10} xs={8}>
              Group Information{" "}
            </Col>
          </div>
          ...
          <div class="groupProfileInfoContainer">
            ...
            <Row
              className="profileGroupLeaveButton"
            ...
            >
              ...
              <Col
                ...
                xs={2}
                onClick={this.props.close}
                dangerouslySetInnerHTML={{ __html: icon_leave_group }}
              />
            </Row>
          </div>
        </div>
  ...
  return (
    <Col lg={12}>
      <Col lg={7} className="profileGroupMemberName">
        <img
          src={
            utils.CheckEmpty(props.member.avatar)
              ? props.member.avatar
              : user_thumbnail
          }
        />
